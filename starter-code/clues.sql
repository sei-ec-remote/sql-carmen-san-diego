...
-- world=# SELECT * FROM countries WHERE region = 'Southern Europe' ORDER BY population LIMIT 1
-- answer VAT
...
-- world=# SELECT continent, code, region FROM countries WHERE code IN ('ITA', 'SMR', 'VAT', 'CHE');
-- ITA OR SMR
-- world=# SELECT name, code  FROM countries WHERE code = 'SMR';
-- world=# SELECT * FROM cities WHERE countrycode = 'SMR';
-- figured out, its san marino
...
-- world=# SELECT continent, countries.name, cities.name FROM countries JOIN cities ON countries.code = cities.countrycode WHERE countries.continent = 'South America' AND cities.name LIKE 'Ser%'; 
-- going with SERRA but could be SERTAOZINHO
